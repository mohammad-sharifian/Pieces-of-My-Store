...
@section('content')
    ...
    <div class="page">
        <div class="page-header">
            <div class="wrapp">
                <h5 class="title"><i class="fas fa-list"></i> لیست محصولات ({{ $products->total() }})</h5>
                <div class="input-g mr-auto">
                    <label for="type_list">دسته بندی نمایش</label>
                    ...
                    <select name="type_list" id="type_list">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}">{{ $category->name }}@isset($category->parent) {{'- ' . $category->parent->name}}@endisset @isset($category->parent->parent) {{'- ' . $category->parent->parent->name}}@endisset</option>
                        @endforeach
                    </select>
                </div>
                ...
                <a href="{{ route('admin.products.create') }}" class="back btn btn-sm btn-outline-primary">
                    ...
                    <span class="add"><span class="res-xs">ایجاد</span> محصول <span class="res-xs">جدید</span></span>
                </a>
            </div>
            <hr>
        </div>
        ...
        <div class="page-content">
            ...
            <div class="table-wr">
                <table class="table table-sm table-striped text-center table-bordered table-responsive-md">
                    <thead class="">
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td class="pt-3">{{ $product->category->name }} @isset($product->category->parent)<span class="text-black-50">{{'- ' . $product->category->parent->name}}</span>@endisset @isset($product->category->parent->parent) <span class="text-black-50">{{'- ' . $product->category->parent->parent->name}}</span>@endisset</td>
                                ...
                                <td class="pt-3">
                                    ...
                                    <button id="btn-collapse-edit-{{ $product->id }}"
                                        class="btn-collapse-edit btn btn-sm btn-outline-primary">
                                        ویرایش
...
@section('script')
    <script>
        ...
        // $.get( `{{ url('admin-panel/management/products/') }}` );
